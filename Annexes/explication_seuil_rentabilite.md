...
Ce document a pour but de vous expliquer, de la manière la plus simple et transparente possible, comment le calcul du "seuil de rentabilité" (aussi appelé "point mort") a été effectué.
...
## 1. SR : Charges Fixes vs. Variables

Pour comprendre le calcul, il faut distinguer deux types de dépenses :

-   **Les Charges Variables :** Ce sont les coûts qui dépendent *directement* du nombre de clients. Dans la restauration, c'est principalement le **coût des matières premières**. Si vous ne servez personne, vous ne dépensez rien en nourriture. Si vous servez 100 clients, vous dépensez 100 fois le coût d'un repas.
...
1.  Une partie sert d'abord à payer les **charges variables** (la nourriture).
    -   22 € * 30% = **6,60 €**
...
## 3. SR : Preuve par le Calcul (26 Couverts)
